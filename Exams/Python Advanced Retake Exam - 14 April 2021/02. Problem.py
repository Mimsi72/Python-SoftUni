...
while True:
    ...
    row = int(shoots_row)
    col = int(shoots_col)
    points_multiplier = ((int(matrix[row][0])) + (int(matrix[row][6])) + (int(matrix[0][col])) + (
        int(matrix[6][col])))
    turns_counter += 1
    ...
    if turns_counter % 2 == 0:
        if 0 <= row < 7 and 0 <= col < 7:
            ...
            if matrix[row][col].isnumeric():
                ...
                player_1_score -= points_multiplier * 2
            elif matrix[row][col] == "T":
                player_1_score -= points_multiplier * 3
        ...
        if 0 <= row < 7 and 0 <= col < 7:
            ...
            if matrix[row][col].isnumeric():
                ...
                player_2_score -= points_multiplier * 2
            elif matrix[row][col] == "T":
                player_2_score -= points_multiplier * 3
            elif matrix[row][col] == "B":
